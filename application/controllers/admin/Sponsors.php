...
class Sponsors extends MY_Controller {
    ...
    public function index() {
        ...
        $sponsors = $this->content->get_content_by_type($this->type, $page);
    ...
    public function view($id) {
        $sponsor = $this->content->get_content_by_id($this->type, $id);
        $data['sponsor'] = $sponsor[0];
        $images = $this->image->get_images_by_content_id($id);

        foreach ($images as $image) {
            $data['sponsor']['images'][] = $image['path'] . $image['name'];
        }
    ...
    public function submit() {
        ...
        redirect(site_url('admin/' . $this->type . '/view/' . $sponsor_id));
    }
