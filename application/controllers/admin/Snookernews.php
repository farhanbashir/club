...
class Snookernews extends MY_Controller {
    ...
    public function index() {
        ...
        $snookernews = $this->content->get_content_by_type($this->type, $page);
        $data['snookernews'] = $snookernews;
        $content = $this->load->view($this->type . '/tabular.php', $data, true);
        $this->load->view('welcome_message', array('content' => $content));
    }

    public function view($id) {
        $snookernew = $this->content->get_content_by_id($this->type, $id);
        $data['snookernew'] = $snookernew[0];
        $images = $this->image->get_images_by_content_id($id);

        foreach ($images as $image) {
            $data['snookernew']['images'][] = $image['path'] . $image['name'];
        }
    ...
    public function update() {
        ...
        $data = array(
            'title' => $_POST['snookernew']['title'],
            'description' => $_POST['snookernew']['description'],
            'detail_description' => $_POST['snookernew']['detail_description'],
        );

        $snookernew_id = $this->content->update_content_by_id($_POST['snookernew']['id'], $data);

        $image_data = $this->uploadImageFile($snookernew_id, $this->type);

        if ($this->uploadSuccess) {
            $this->image->delete_content_images($snookernew_id);
            $this->image->add_images($image_data);
        }

        redirect(site_url('admin/page/snooker'));
    }
    ...
    public function submit() {
        ...
        $snookernew_id = $this->content->add_content($data, $this->type);
        $image_data = $this->uploadImageFile($snookernew_id, $this->type);
