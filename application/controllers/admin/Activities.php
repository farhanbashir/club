...
class Activities extends My_Controller {
    ...
    public function index() {
        ...
        $activities = $this->content->get_content_by_type($this->type, $page);
    ...
    public function view($id) {
        $activity = $this->content->get_content_by_id($this->type, $id);
        $data['activity'] = $activity[0];
        $images = $this->image->get_images_by_content_id($id);

        foreach ($images as $image) {
            $data['activity']['images'][] = $image['path'] . $image['name'];
        }
    ...
    public function update() {
        ...
        $image_data = $this->uploadImageFile($activity_id, $this->type);

        if ($this->uploadSuccess) {
            $this->image->add_images($image_data);
        }

        redirect(site_url('admin/' . $this->type . '/edit/' . $activity_id));
    }
    ...
    public function submit() {
        ...
        redirect(site_url('admin/' . $this->type . '/view/' . $activity_id));
    }
